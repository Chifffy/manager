...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusszhiyuanzhezhaomu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusszhiyuanzhezhaomu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusszhiyuanzhezhaomuCrossAddOrUpdateFlag = false;
						 this.parent.search();
