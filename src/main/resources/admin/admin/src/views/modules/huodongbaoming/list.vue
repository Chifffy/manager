<template>
	<div class="main-content" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"3px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">活动名称</label>
						<el-input v-model="searchForm.huodongmingcheng" placeholder="活动名称" clearable></el-input>
					</div>
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(64, 158, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('huodongbaoming','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('huodongbaoming','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="huodongtupian" width="200" label="活动图片">
						<template slot-scope="scope">
							<div v-if="scope.row.huodongtupian">
								<img v-if="scope.row.huodongtupian.substring(0,4)=='http'" :src="scope.row.huodongtupian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.huodongtupian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('huodongbaoming','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #3ca512","cursor":"pointer","padding":"0 10px 0 24px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20221011/ca1c191554d24b108bc94f4a2046d636.png) #41b314 no-repeat 5px 8px","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('huodongbaoming','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid #3ca512","cursor":"pointer","padding":"0 10px 0 24px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20221011/ca1c191554d24b108bc94f4a2046d636.png) #41b314 no-repeat 5px 8px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('huodongbaoming','活动评价')" type="success" size="mini" @click="huodongpingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','','')">活动评价</el-button>
							<el-button :style='{"border":"1px solid #00a0f0","cursor":"pointer","padding":"0 10px 0 24px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20221011/161eb7a46f5d4cd19d68a1386174d662.png) #00aaff no-repeat 5px 8px","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('huodongbaoming','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px 0 24px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20221011/68bd264a8e4341c6aa5409f871d590d0.png) #d9534f no-repeat 5px 8px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('huodongbaoming','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<huodongpingjia-cross-add-or-update v-if="huodongpingjiaCrossAddOrUpdateFlag" :parent="this" ref="huodongpingjiaCrossaddOrUpdate"></huodongpingjia-cross-add-or-update>
...
import huodongpingjiaCrossAddOrUpdate from "../huodongpingjia/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      huodongpingjiaCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    huodongpingjiaCrossAddOrUpdate,
  },
  methods: {
    ...
    huodongpingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.huodongpingjiaCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','huodongbaoming');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.huodongpingjiaCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.huodongpingjiaCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          huodongdidian: row.huodongdidian,
          huodongtupian: row.huodongtupian,
