	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="年级班级" prop="nianjibanji">
					<el-input v-model="ruleForm.nianjibanji" placeholder="年级班级" clearable  :readonly="ro.nianjibanji"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="年级班级" prop="nianjibanji">
					<el-input v-model="ruleForm.nianjibanji" placeholder="年级班级" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="证件号码" prop="zhengjianhaoma">
					<el-input v-model="ruleForm.zhengjianhaoma" placeholder="证件号码" clearable  :readonly="ro.zhengjianhaoma"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="证件号码" prop="zhengjianhaoma">
					<el-input v-model="ruleForm.zhengjianhaoma" placeholder="证件号码" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="政治面貌" prop="zhengzhimianmao">
					<el-input v-model="ruleForm.zhengzhimianmao" placeholder="政治面貌" clearable  :readonly="ro.zhengzhimianmao"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="政治面貌" prop="zhengzhimianmao">
					<el-input v-model="ruleForm.zhengzhimianmao" placeholder="政治面貌" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.gerenjianjie" label="个人简介" prop="gerenjianjie">
					<file-upload
						tip="点击上传个人简介"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.gerenjianjie?ruleForm.gerenjianjie:''"
						@change="gerenjianjieUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.gerenjianjie" label="个人简介" prop="gerenjianjie">
					<el-button :style='{"border":"1px solid #47afce","cursor":"pointer","padding":"0 35px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20221011/adab350062104c0cb8cb8d542f54567f.png) #5bc0de no-repeat 15px 11px","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.gerenjianjie)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.gerenjianjie" label="个人简介" prop="gerenjianjie">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='nianjibanji'){
							this.ruleForm.nianjibanji = obj[o];
							this.ro.nianjibanji = true;
							continue;
						}
						if(o=='zhengjianhaoma'){
							this.ruleForm.zhengjianhaoma = obj[o];
							this.ro.zhengjianhaoma = true;
							continue;
						}
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.nianjibanji!=''&&json.nianjibanji) || json.nianjibanji==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.nianjibanji = json.nianjibanji
						this.ro.nianjibanji = true;
					}
				} else {
	...
	if(this.ruleForm.gerenjianjie!=null) {
		this.ruleForm.gerenjianjie = this.ruleForm.gerenjianjie.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhiyuanzhebaoming/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhiyuanzhebaomingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    gerenjianjieUploadChange(fileUrls) {
	    this.ruleForm.gerenjianjie = fileUrls;
    },
  }
};
</script>
