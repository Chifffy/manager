	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="活动内容" prop="huodongneirong">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.huodongneirong" label="活动内容" prop="huodongneirong">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.huodongneirong"></span>
                </el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="报名要求" prop="baomingyaoqiu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.baomingyaoqiu" label="报名要求" prop="baomingyaoqiu">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.baomingyaoqiu"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../springboot7cox6/upload');
        this.ruleForm.baomingyaoqiu = this.ruleForm.baomingyaoqiu.replace(reg,'../../../springboot7cox6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhiyuanzhehuodong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhiyuanzhehuodongCrossAddOrUpdateFlag = false;
						 this.parent.search();
