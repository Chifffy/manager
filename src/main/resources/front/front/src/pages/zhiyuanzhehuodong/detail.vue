	...
	<div class="detail-preview" :style='{"width":"100%","padding":"10px 7% 20px","margin":"0px auto 0","position":"relative","background":"none"}'>
		<div class="attr" :style='{"border":"1px solid #ddd","minHeight":"620px","padding":"10px 0 0px 0px","boxShadow":"0px 0px 4px #eee","overflow":"hidden","flexWrap":"wrap","background":"rgba(255,255,255,1)","display":"flex","position":"relative","justifyContent":"space-between"}'>
			...
			<div class="info" :style='{"minHeight":"500px","width":"48%","padding":"10px 10px 10px 20px","margin":"0","background":"none"}'>
				...
				<div class="item" :style='{"padding":"0","margin":"0 0 15px 0","borderColor":"#c6ecc6","borderWidth":"0px 0px 1px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"right","background":"linear-gradient(90deg, rgba(255,255,255,1) 0%, rgba(230,245,229,1) 100%),#e6f5e5","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>开始时间</div>
					<div  :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","background":"linear-gradient(270deg, rgba(255,255,255,1) 0%, rgba(230,245,229,1) 100%),#e6f5e5"}'>{{detail.kaishishijian}}</div>
				</div>
				<div class="item" :style='{"padding":"0","margin":"0 0 15px 0","borderColor":"#c6ecc6","borderWidth":"0px 0px 1px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"right","background":"linear-gradient(90deg, rgba(255,255,255,1) 0%, rgba(230,245,229,1) 100%),#e6f5e5","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>结束时间</div>
					<div  :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","background":"linear-gradient(270deg, rgba(255,255,255,1) 0%, rgba(230,245,229,1) 100%),#e6f5e5"}'>{{detail.jieshushijian}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#e1effa","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="isAuth('zhiyuanzhehuodong','报名')" @click="onAcross('huodongbaoming','','','')" type="warning">报名</el-button>
				</div>
			</div>
		...
		<el-tabs class="detail" :style='{"border":"4px solid #c7eec2","boxShadow":"0 0px 0px rgb(0 0 0 / 30%)","padding":"10px","margin":"20px 0 0","borderRadius":"2px","background":"rgba(255,255,255,1)"}' v-model="activeName" type="border-card">
			...
			<el-tab-pane label="报名要求" name="first1">
				<div v-html="detail.baomingyaoqiu"></div>
			</el-tab-pane>
  ...
  export default {
    ...
    methods: {
      ...
      onAcross(acrossTable,crossOptAudit,statusColumnName,tips,statusColumnValue){
        localStorage.setItem('crossTable',`zhiyuanzhehuodong`);
      ...
      storeup(type) {
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'zhiyuanzhehuodong', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discusszhiyuanzhehuodong/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusszhiyuanzhehuodong/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
